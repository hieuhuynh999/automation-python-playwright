...
class EtmsPricingReportPage(EtmsCatalogueMenuPage):
    ...
    def _wait_for_download_enabled(self, timeout: int | None = None) -> None:
        timeout = timeout or settings.browser_timeout
        deadline = time.monotonic() + timeout / 1000
        while time.monotonic() < deadline:
        ...
        raise AssertionError(
            f"{self.title} Download control not enabled after {timeout}ms. "
